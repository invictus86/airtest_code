...
def auto_xshell_input():
    ...
    cmd21 = "setenv setssiargs 'setenv bootargs init=/init console=null,115200 ${mtdparts}\;${sfcparts} root=/dev/mtdblock_robbs11 rootfstype=squashfs ro'"
    ...
    cmd24 = "setenv setfsiargs 'setenv bootargs init=/init console=null,115200 ${mtdparts}\;${sfcparts} root=/dev/mtdblock_robbs9 rootfstype=squashfs ro'"
    ...
    cmd27 = "setenv bootargs 'init=/init console=null,115200 mtdparts=hinand:16M(trustedcore),16M(trustedcorebak),4M(baseparam),4M(pqparam),4M(logo),4M(deviceinfo),16M(kernel),300M(rootfs),16M(upg_kernel),55M(upg_rootfs),4M(serial),64M(user),1M(fdt),4M(cadata),-(others);hi_sfc:1408K(fastboot),256K(bootenv) root=/dev/mtdblock_robbs9 rootfstype=squashfs ro'"
